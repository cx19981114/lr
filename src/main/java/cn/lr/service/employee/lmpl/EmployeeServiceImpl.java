...
public class EmployeeServiceImpl implements EmployeeService {
	...
	public Integer addEmployee(JSONObject data){
		...
		if (data.getInteger("leaderId") != null) {
			...
			if (test == null || test.getState() == dictMapper.selectByCodeAndStateName(DATA_TYPE, "已失效",data.getInteger("companyId"))) {
	...
	public Integer modifyEmployee(JSONObject data){
		...
		if (data.getInteger("companyId") != null) {
			...
			if (company == null || company.getState() == dictMapper.selectByCodeAndStateName(DATA_TYPE, "已失效",data.getInteger("companyId"))) {
				throw new BusiException("该companyId不存在");
			}
			record.setCompanyId(data.getInteger("companyId"));
		}
		if (data.getInteger("leaderId") != null) {
	...
	public Integer deleteEmployee(JSONObject data) throws Exception {
		...
		if(record.getLeaderIdList() != null && !"".equals(record.getLeaderIdList())) {
			...
			List<customerProject> customerProjects = customerProjectMapper.selectByEmployee(record.getId(), stateSX, 0, Integer.MAX_VALUE);
			...
			List<customerPerformance> customerPerformances = customerPerformanceMapper.selectByEmployee(record.getId(), stateSX, 0, Integer.MAX_VALUE);
			for(customerPerformance c:customerPerformances) {
				if(c.getState() == stateCG) {
					...
					if(c.getType() == dictMapper.selectByCodeAndStateName(CUSTOMERPERFORMAN_TYPE, "新增顾客", record.getCompanyId())) {
					...
					if(c.getType() == dictMapper.selectByCodeAndStateName(CUSTOMERPERFORMAN_TYPE, "新增顾客", record.getCompanyId())) {
						...
						oJsonObject.put("type", dictMapper.selectByCodeAndStateCode(CUSTOMERPERFORMAN_TYPE, c.getType(), record.getCompanyId()));
						CustomerProjectService.addCustomerProject(oJsonObject);
					}
					JSONObject oJsonObject = new JSONObject();
					oJsonObject.put("customerProjectId", c.getId());
	...
	public employeeDTO getEmployee(JSONObject data) {
		...
		employeeDTO.setEmployeeId(record.getId());
		employeeDTO.setCompanyId(record.getCompanyId());
		employeeDTO.setPostId(record.getPostId());
		employeeDTO.setName(record.getName());
		employeeDTO.setPhone(record.getPhone());
		employeeDTO.setPic(record.getPic());
		employeeDTO.setPost(postMapper.selectByPrimaryKey(record.getPostId()).getName());
		employeeDTO.setSex(record.getSex());
		employeeDTO.setState(dictMapper.selectByCodeAndStateCode(EMPLOYEE_TYPE, record.getState(),data.getInteger("companyId")));
		System.out.println(record.getLeaderIdList());
	...
	public Page<employeeDTO> getEmployeeByCompany(JSONObject data) {
		...
		List<employee> employees = employeeMapper.selectByCompanyId(companyId,state, (pageNum - 1) * PAGESIZE, PAGESIZE);
		int total = employeeMapper.selectByCompanyCount(companyId,state);
		List<employeeDTO> employeeDTOs = new ArrayList<employeeDTO>();
		for (employee e : employees) {
			employeeDTO employeeDTO = new employeeDTO();
			employeeDTO.setCompany(companyMapper.selectByPrimaryKey(e.getCompanyId()).getName());
			employeeDTO.setEmployeeId(e.getId());
			employeeDTO.setCompanyId(e.getCompanyId());
			employeeDTO.setPostId(e.getPostId());
			employeeDTO.setName(e.getName());
			employeeDTO.setPhone(e.getPhone());
			employeeDTO.setPic(e.getPic());
			employeeDTO.setPost(postMapper.selectByPrimaryKey(e.getPostId()).getName());
			employeeDTO.setSex(e.getSex());
			employeeDTO.setState(dictMapper.selectByCodeAndStateCode(EMPLOYEE_TYPE, e.getState(),data.getInteger("companyId")));
			if (e.getLeaderIdList() != null && !e.getLeaderIdList().equals("")) {
				String leaderId = e.getLeaderIdList().split("-")[0];
				employee employee = employeeMapper.selectByPrimaryKey(Integer.valueOf(leaderId));
				if (employee == null || employee.getState() == dictMapper.selectByCodeAndStateName(DATA_TYPE, "已失效",data.getInteger("companyId"))) {
					throw new BusiException("该employeeId不存在");
				}
	...
	public Page<employeeDTO> getEmployeeByPost(JSONObject data) {
		...
		Integer pageNum = data.getInteger("pageNum");
		Integer state = dictMapper.selectByCodeAndStateName(DATA_TYPE, "已失效", data.getInteger("companyId"));
		List<employee> employees = employeeMapper.selectByPostId(companyId, postId,state, (pageNum - 1) * PAGESIZE, PAGESIZE);
	...
	public List<JSONObject> getEmployeeLeader(JSONObject data) {
		post post = postMapper.selectByPrimaryKey(data.getInteger("postId"));
		Integer state = dictMapper.selectByCodeAndStateName(DATA_TYPE, "已失效", data.getInteger("companyId"));;
		List<employee> employees = employeeMapper.selectByPostId(data.getInteger("companyId"), post.getLeaderPostId(),state,
				0, Integer.MAX_VALUE);
	...
	public List<JSONObject> getEmployeeUnder(JSONObject data) {
		...
		for (int i = 0; i < underIdList.length; i++) {
			...
			if (employee2 == null || employee2.getState() == dictMapper.selectByCodeAndStateName(DATA_TYPE, "已失效",data.getInteger("companyId"))) {
				throw new BusiException("该employeeId不存在");
			}
	...
	public JSONObject getDayMsg(JSONObject data) {
		...
		if(employeeRest != null) {
			dayJson.put("rest", dictMapper.selectByCodeAndStateCode(ATTENDANCE_TYPE, employeeRest.getSchedule(), data.getInteger("companyId")));
		}else {
			dayJson.put("rest", "暂无");
		}
		return dayJson;
